...
async function run() {
  try {
    ...
    const moveOnClose = core.getInput('moveonclose');
    const removeOnClose = core.getInput('removeonclose');
    const start = new Date();
    core.debug('calling handler');
    await handler(token, owner, repo, project, columnByLabelStr, ignoreColumnNamesStr, labelOnClose, moveOnClose, removeOnClose);
